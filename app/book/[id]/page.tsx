...
export default function BookMentorPage() {
  ...
  useEffect(() => {
    if (!id) return;
    supabaseClient.from("mentors_public").select("id,name,headline,calendly,calendly_url").eq("id", id).single().then(({ data, error }) => {
