...
export default function SettingsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (!user) {
        window.location.replace("/auth?next=/settings");
        return;
      }
      setUserId(user.id);
      setEmail(user.email || user.phone || "");
      const { data } = await supabase.from("profiles").select("full_name,age,phone,city,occupation,bio").eq("id", user.id).maybeSingle();
      if (data) {
        setForm({
          full_name: String(data.full_name || ""), age: data.age ? String(data.age) : "", phone: String(data.phone || ""),
          city: String(data.city || ""), occupation: String(data.occupation || ""), bio: String(data.bio || "")
        });
      }
      setLoading(false);
    }
  ...
  async function save(event: FormEvent) {
    ...
    else setMessage("Your profile has been updated.");
    setSaving(false);
  }
  ...
  return <main className="min-h-screen bg-paper text-ink px-4 py-8 sm:px-6 sm:py-14">
    <div className="mx-auto max-w-4xl">
      ...
      <div className="mt-8 max-w-2xl">
        ...
        <p className="mt-6 font-mono text-xs uppercase tracking-[0.16em] text-board/60">ACCOUNT SETTINGS</p>
        <h1 className="mt-2 font-display text-4xl sm:text-5xl">Keep your profile up to date.</h1>
        <p className="mt-4 text-base leading-7 text-ink/60">Your basic profile helps keep your AglaKadam account organized. Only share details you are comfortable providing.</p>
      </div>

      <form onSubmit={save} className="mt-8 max-w-3xl rounded-sm border border-ink/10 bg-white p-6 pin-shadow sm:p-8">
        <div className="mb-7 flex items-start gap-3 rounded-sm bg-board/5 p-4 text-sm text-ink/65">
          <CheckCircle2 size={18} className="mt-0.5 shrink-0 text-board" />
          <p>Your sign-in email is managed by your account and cannot be changed here.</p>
        </div>
        <div className="grid gap-5 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            <label className="mb-2 block font-mono text-xs uppercase tracking-[0.1em] text-ink/60">A little about you <span className="text-ink/35">(optional)</span></label>
            <textarea value={form.bio} onChange={(e) => update("bio", e.target.value)} maxLength={500} rows={5} className="input" placeholder="What are you currently working on or figuring out?" />
            <p className="mt-1 text-right text-xs text-ink/35">{form.bio.length}/500</p>
          </div>
        </div>
        {error && <p className="mt-5 rounded-sm bg-red-50 p-3 text-sm text-red-700" role="alert">{error}</p>}
        {message && <p className="mt-5 rounded-sm bg-board/10 p-3 text-sm text-board" role="status">{message}</p>}
        <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
          <Link href="/dashboard" className="inline-flex items-center justify-center rounded-sm border border-ink/15 px-5 py-3 text-sm hover:bg-ink/5">Cancel</Link>
          <button type="submit" disabled={saving} className="inline-flex items-center justify-center gap-2 rounded-sm bg-amber px-5 py-3 font-semibold disabled:opacity-60"><Save size={16}/>{saving ? "Saving…" : "Save changes"}</button>
        </div>
      </form>
    </div>
  </main>;
...
function Field({ label, value, onChange, required, type = "text", placeholder, readOnly, min, max }: { label: string; value: string; onChange?: (value: string) => void; required?: boolean; type?: string; placeholder?: string; readOnly?: boolean; min?: string; max?: string }) {
  return <div>
    <label className="mb-2 block font-mono text-xs uppercase tracking-[0.1em] text-ink/60">{label}{required && <span className="text-amber"> *</span>}</label>
    <input value={value} onChange={(e) => onChange?.(e.target.value)} type={type} placeholder={placeholder} readOnly={readOnly} required={required} min={min} max={max} maxLength={type === "number" ? undefined : 120} className={`input ${readOnly ? "bg-paper text-ink/55" : ""}`} />
  </div>;
}
