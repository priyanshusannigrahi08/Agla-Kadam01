    ...
    <main className="min-h-screen bg-paper text-ink">
      <header className="sticky top-0 z-30 border-b border-ink/10 bg-paper/95 backdrop-blur">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 sm:px-6">
          <Link href="/" className="font-display text-xl tracking-tight" aria-label="AglaKadam home">
            AglaKadam
          </Link>
          ...
          <div className="flex items-center gap-2 sm:gap-3">
            ...
            <Link
              href="/find-mentor"
              className="inline-flex items-center justify-center rounded-sm bg-amber px-4 py-2.5 font-body text-sm font-semibold transition hover:brightness-95 sm:px-5"
            >
              Get started
            </Link>
          </div>
      ...
      <section className="relative overflow-hidden bg-board text-chalk">
        ...
        <div className="relative mx-auto grid max-w-7xl gap-12 px-5 pb-16 pt-14 sm:px-6 sm:pb-20 sm:pt-20 lg:grid-cols-[1.15fr_.85fr] lg:items-center lg:gap-16 lg:pt-24">
          <div>
            <p className="mb-5 font-mono text-[11px] uppercase tracking-[0.22em] text-sage">
              “Agla kadam” — your next step
            </p>
            <h1 className="max-w-3xl font-display text-5xl leading-[0.98] tracking-tight sm:text-6xl lg:text-7xl">
              The right conversation can change what comes next.
            </h1>
            <p className="mt-6 max-w-2xl font-body text-base leading-relaxed text-chalk/70 sm:text-lg">
              Find someone who has already navigated the kind of decision you’re facing — then have one focused conversation with them.
            </p>

            <div className="mt-8 max-w-2xl">
              <SearchBar />
              <div className="mt-4 flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.12em] text-chalk/45">
                <Search size={12} /> Search by field, role, skill or problem
              </div>
            </div>
            ...
            <div className="mt-8 flex flex-wrap gap-2">
              {SITUATIONS.slice(0, 4).map((situation) => (
                <Link
                  key={situation.query}
                  href={`/mentors?q=${encodeURIComponent(situation.query)}`}
                  className="rounded-full border border-chalk/20 px-3.5 py-1.5 font-mono text-[10px] uppercase tracking-[0.08em] text-chalk/60 transition hover:border-chalk/50 hover:text-chalk"
                >
                  {situation.label}
                </Link>
              ))}
          ...
          <div className="hidden lg:block">
            <div className="relative ml-auto max-w-sm rotate-1 rounded-sm border border-chalk/15 bg-paper p-7 text-ink shadow-2xl">
              ...
              <div className="mt-7 border-t border-ink/10 pt-5">
                <p className="font-display text-lg italic text-ink/75">“You don’t need all the answers. Sometimes you need the right person to ask.”</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-5 py-16 sm:px-6 sm:py-20">
        <div className="mb-9 flex items-end justify-between gap-6">
          ...
          <Link href="/find-mentor" className="hidden shrink-0 items-center gap-1 font-mono text-xs uppercase tracking-[0.1em] text-board hover:text-board/70 sm:inline-flex">
            Guided matching <ArrowRight size={14} />
          </Link>
        </div>
          ...
          {SITUATIONS.map((situation) => {
            ...
            return (
              <Link
                key={situation.query}
                href={`/mentors?q=${encodeURIComponent(situation.query)}`}
                className="group flex min-h-32 flex-col justify-between rounded-sm border border-ink/10 bg-white p-5 transition hover:-translate-y-0.5 hover:border-board/30 hover:shadow-lg"
              >
                <div className="flex items-start justify-between gap-4">
                  <span className="flex h-9 w-9 items-center justify-center rounded-full bg-board/10 text-board"><Icon size={18} /></span>
                  <ArrowRight size={16} className="text-ink/30 transition group-hover:translate-x-1 group-hover:text-board" />
                </div>
                <p className="mt-7 font-display text-lg">{situation.label}</p>
              </Link>
            );
          })}
        </div>
        ...
        <div className="mx-auto max-w-7xl px-5 py-16 sm:px-6 sm:py-20">
          <div className="grid gap-10 lg:grid-cols-[.75fr_1.25fr] lg:items-end">
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              {STEPS.map((step) => (
                <div key={step.number} className="rounded-sm border border-ink/10 bg-paper p-5">
                  <p className="font-mono text-xs text-amber">{step.number}</p>
                  <h3 className="mt-7 font-display text-xl leading-tight">{step.title}</h3>
      ...
      <section className="mx-auto max-w-7xl px-5 py-16 sm:px-6 sm:py-20">
        <div className="mb-9 flex items-end justify-between gap-6">
          <div>
            <p className="mb-3 font-mono text-xs uppercase tracking-[0.2em] text-board/60">Explore by field</p>
            <h2 className="font-display text-3xl sm:text-4xl">Find experience close to where you want to go.</h2>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
          {FIELDS.map((field) => {
            ...
            return (
              <Link key={field.query} href={`/mentors?q=${encodeURIComponent(field.query)}`} className="group rounded-sm border border-ink/10 bg-white p-5 transition hover:-translate-y-0.5 hover:border-board/30">
                <Icon size={21} className="text-board" />
                <p className="mt-8 font-body text-sm font-medium leading-snug">{field.label}</p>
                <ArrowRight size={14} className="mt-3 text-ink/25 transition group-hover:translate-x-1 group-hover:text-board" />
              </Link>
            );
          })}
        </div>
      ...
      <section className="border-b border-ink/10 bg-white">
        <div className="mx-auto max-w-2xl px-5 py-14 text-center sm:px-6 sm:py-16">
          <p className="mb-4 font-mono text-xs uppercase tracking-[0.2em] text-board/60">A growing community</p>
      ...
      <section className="bg-board text-chalk">
        <div className="mx-auto flex max-w-7xl flex-col gap-7 px-5 py-16 sm:px-6 sm:py-20 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="mb-3 font-mono text-xs uppercase tracking-[0.2em] text-sage">For people with experience to share</p>
            <h2 className="max-w-2xl font-display text-3xl leading-tight sm:text-4xl">Someone is trying to figure out what you already learned the hard way.</h2>
          </div>
          <Link href="/mentor" className="inline-flex shrink-0 items-center justify-center gap-2 rounded-sm bg-amber px-6 py-3.5 font-body font-semibold text-ink transition hover:brightness-95">
      ...
      <footer className="bg-ink text-chalk/70">
        ...
        <div className="border-t border-chalk/10">
          <p className="mx-auto max-w-7xl px-5 py-6 font-mono text-xs text-chalk/40 sm:px-6">AglaKadam — built in the open.</p>
        </div>
      </footer>
    </main>
  );
...
function QuickCard({ icon: Icon, title, body, href }: { icon: typeof Users; title: string; body: string; href: string }) {
  return (
    <Link href={href} className="group pin-shadow rounded-sm border border-ink/10 bg-white p-5 transition hover:-translate-y-1">
      <div className="flex items-start justify-between gap-4">
        <Icon size={22} className="text-board" />
        <ArrowRight size={15} className="text-ink/25 transition group-hover:translate-x-1 group-hover:text-board" />
      </div>
...
function FooterColumn({ title, links }: { title: string; links: { label: string; href: string }[] }) {
  return (
    <div>
      <p className="mb-4 font-mono text-xs uppercase tracking-[0.15em] text-chalk/40">{title}</p>
      <ul className="space-y-2">
        {links.map((link) => (
          <li key={link.href}>
            <Link href={link.href} className="font-body text-sm hover:text-chalk">{link.label}</Link>
          </li>
        ))}
      </ul>
    </div>
