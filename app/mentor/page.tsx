...
export default function MentorSignup() {
  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const { data: mentorRow, error: insertError } = await supabase.from("mentors").insert({
        user_id: user.id,
        name,
        email,
        linkedin,
      ...
      const mentor = mentorRow as { id: string } | null;
      ...
      <main className="min-h-screen bg-paper text-ink">
        <div className="mx-auto max-w-3xl px-5 py-8 sm:px-8 sm:py-12 lg:py-16">
          ...
          <section className="mt-8 rounded-sm border border-ink/10 bg-white p-6 shadow-[0_20px_60px_-45px_rgba(23,34,28,0.7)] sm:p-8">
            <div className="flex items-start justify-between gap-5"><div><p className="font-mono text-[10px] uppercase tracking-[0.16em] text-board/60">AI profile intelligence</p><h2 className="mt-2 font-display text-2xl">{assessment.readiness_label}</h2><p className="mt-2 text-sm text-ink/50">Profile readiness based on the information you provided and available evidence.</p></div><div className="shrink-0"><span className="font-display text-4xl">{assessment.readiness_score}</span><span className="text-sm text-ink/40"> / 100</span></div></div>
    ...
    <main className="min-h-screen bg-paper text-ink">
      <div className="mx-auto max-w-3xl px-5 py-8 sm:px-8 sm:py-12 lg:py-16">
        ...
        <form onSubmit={handleSubmit} className="mt-10 overflow-hidden rounded-sm border border-ink/10 bg-white shadow-[0_20px_60px_-45px_rgba(23,34,28,0.7)] sm:mt-12">
          ...
          <div className="space-y-8 px-6 py-7 sm:px-8 sm:py-9"><Field label="What can you actually speak to?" htmlFor="expertise" required hint="Be specific — this helps us make better matches."><textarea id="expertise" name="expertise" required maxLength={2000} rows={5} className="form-input resize-y" placeholder="For example: breaking into product management, moving from consulting to a startup, preparing for analytics interviews, or navigating your first year as a manager." /><p className="mt-2 text-xs text-ink/40">Think in terms of situations, decisions, skills or transitions you’ve personally experienced.</p></Field><Field label="Your experience" htmlFor="experience" optional hint="A quick snapshot is enough."><input id="experience" name="experience" maxLength={500} className="form-input" placeholder="E.g. 7 years · Senior PM · B2B SaaS · Mumbai" /></Field><Field label="Your career journey" htmlFor="journey" optional hint="What turns or lessons might be useful to someone following a similar path?"><textarea id="journey" name="journey" maxLength={2000} rows={5} className="form-input resize-y" placeholder="A few lines about where you started, the turns you made, and what you learned along the way." /></Field></div>
          ...
          <div className="border-t border-ink/10 bg-paper px-6 py-6 sm:px-8 sm:py-7">{error && <div className="mb-5 rounded-sm border border-red-200 bg-red-50 px-4 py-3 text-sm leading-6 text-red-700" role="alert">{error}</div>}{message && <div className="mb-5 rounded-sm border border-board/15 bg-board/[0.035] px-4 py-3 text-sm leading-6 text-board" role="status">{message}</div>}<div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><p className="max-w-md text-xs leading-5 text-ink/45">Submit once. AglaKadam will save your profile, securely upload your evidence, and run the AI profile review before showing you the result.</p><button type="submit" disabled={submitting} className="inline-flex min-h-12 shrink-0 items-center justify-center gap-2 rounded-sm bg-amber px-7 py-3.5 font-semibold text-ink shadow-sm transition hover:-translate-y-0.5 hover:shadow-md disabled:cursor-not-allowed disabled:opacity-60">{submitting ? <><Loader2 size={16} className="animate-spin" /> Reviewing…</> : <>Submit & review profile <Sparkles size={16} /></>}</button></div></div>
        </form>
...
function Field({ label, htmlFor, children, required, optional, hint }: { label: string; htmlFor: string; children: ReactNode; required?: boolean; optional?: boolean; hint?: string }) { return <div><label htmlFor={htmlFor} className="flex items-baseline justify-between gap-4"><span className="font-mono text-xs uppercase tracking-[0.11em] text-ink/65">{label}</span>{required ? <span className="text-[11px] text-board">Required</span> : optional ? <span className="text-[11px] text-ink/35">Optional</span> : null}</label>{hint && <p className="mt-1.5 text-xs leading-5 text-ink/45">{hint}</p>}<div className="mt-2.5">{children}</div></div>; }
function TrustItem({ icon, text }: { icon: ReactNode; text: string }) { return <div className="flex items-center gap-2 rounded-sm border border-ink/10 bg-white px-3 py-2.5 text-xs text-ink/60"><span className="text-board">{icon}</span>{text}</div>; }
function ResultPanel({ title, children }: { title: string; children: ReactNode }) { return <div className="rounded-sm border border-ink/10 bg-paper p-4"><p className="font-mono text-[10px] uppercase tracking-[0.12em] text-board/60">{title}</p><div className="mt-3">{children}</div></div>; }
