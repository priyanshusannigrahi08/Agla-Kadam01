...
export default function BookMentorPage() {
  ...
  async function confirmBooking() {
    ...
    if (!user) { router.push(`/auth?next=/book/${id}`); return; }
    const bookingUrl = mentor?.calendly || mentor?.calendly_url || null;
    const { error: insertError } = await supabase.from("bookings").insert({ mentee_user_id: user.id, mentor_id: id, duration_minutes: 30, status: "confirmed", booking_url: bookingUrl });
