...
export default function AdminPage() {
  ...
  return <main className="min-h-screen bg-paper text-ink"><div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12">
    ...
    {tab === "mentors" && <Panel title="Mentor applications" eyebrow="MENTORS"><div className="space-y-4">{data.mentors.length === 0 ? <Empty text="No mentor profiles yet."/> : data.mentors.map((m) => <MentorReviewCard key={m.id} mentor={m} documents={documentsByMentor[m.id] || []} assessment={assessmentsByMentor[m.id]} busy={busy === `mentor:${m.id}`} onUpdate={update}/>)}</div></Panel>}
...
function MentorReviewCard({ mentor, documents, assessment, busy, onUpdate }: { mentor: Mentor; documents: MentorDocument[]; assessment?: Assessment; busy: boolean; onUpdate: (resource: "mentor" | "review" | "booking", id: string, status: string, verification_status?: string) => void }) {
