...
type MentorProfile = { id: string; name: string; headline?: string; bio?: string; expertise?: string; experience?: string; company?: string; role?: string; location?: string; photo_url?: string };
...
export async function POST(request: NextRequest) {
  try {
    ...
    const admin = getSupabaseAdmin(); const { data, error } = await admin.from("mentors_public").select("id,name,headline,bio,expertise,experience,company,role,location,photo_url").limit(MAX_MENTORS);
