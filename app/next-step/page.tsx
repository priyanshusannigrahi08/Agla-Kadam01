...
type NextItem = { kind: "goal" | "action" | "conversation"; title: string; detail: string; href: string };
...
export default function NextStepPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      if (goal) {
        setItem({ kind: "goal", title: goal.text, detail: "Your active career goal. Keep it small enough to act on today.", href: "/goals" });
        setLoading(false); return;
      }

      const action = (actionResult.data || [])[0] as Row | undefined;
      if (action) {
        setItem({ kind: "action", title: action.title, detail: action.description || "An unfinished action from a mentoring conversation.", href: action.booking_id ? `/conversation/${action.booking_id}` : "/progress" });
        setLoading(false); return;
      }

      const booking = (bookingResult.data || [])[0] as Row | undefined;
      if (booking) {
        setItem({ kind: "conversation", title: "Prepare for your next mentor conversation", detail: booking.scheduled_for ? `Your next conversation is ${new Intl.DateTimeFormat("en-IN", { weekday: "long", day: "numeric", month: "short", hour: "numeric", minute: "2-digit" }).format(new Date(booking.scheduled_for))}.` : "Your mentor conversation is active. Bring one decision and one question.", href: `/book/${booking.mentor_id}` });
      }
      setLoading(false);
    }
    load();
  ...
  return <main className="min-h-screen bg-paper text-ink"><div className="mx-auto max-w-3xl px-6 py-10 sm:py-16"><Link href="/dashboard" className="font-mono text-xs uppercase tracking-[0.15em] text-board/60">← Dashboard</Link><div className="mt-14 text-center"><div className="mx-auto flex h-14 w-14 items-center justify-center rounded-full bg-board/10 text-board">{item ? icon : <Zap size={21}/>}</div><p className="mt-7 font-mono text-[10px] uppercase tracking-[0.18em] text-board/60">Today’s next step</p><h1 className="mx-auto mt-3 max-w-2xl font-display text-4xl leading-tight sm:text-6xl">{item?.title || "Choose one small thing to move forward."}</h1><p className="mx-auto mt-5 max-w-xl text-sm leading-7 text-ink/55 sm:text-base">{item?.detail || "Start with a goal, find a mentor, or use the Career Navigator to turn uncertainty into an experiment."}</p>{item ? <Link href={item.href} className="mt-8 inline-flex items-center gap-2 rounded-sm bg-amber px-7 py-3.5 text-sm font-semibold">Do this next <ArrowRight size={15}/></Link> : <div className="mt-8 flex flex-wrap justify-center gap-2"><Link href="/goals" className="inline-flex items-center gap-2 rounded-sm bg-amber px-6 py-3 text-sm font-semibold">Set a goal <ArrowRight size={14}/></Link><Link href="/find-mentor" className="inline-flex items-center gap-2 rounded-sm border border-ink/15 px-6 py-3 text-sm font-semibold">Find a mentor</Link></div>}</div><div className="mt-20 border-t border-ink/10 pt-6 text-center"><p className="text-xs text-ink/40">AglaKadam helps you choose a next step; you remain in control of the decision.</p></div></div></main>;
}
