  ...
  async function handleSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from("mentors").insert({
        ...
        why_mentor: form.get("why_mentor") || "",
        calendly: form.get("calendly"),
        photo_url,
      });
...
<Field label="Your name" htmlFor="name"><input id="name" name="name" required className="input" placeholder="Arjun Mehta"/></Field><Field label="Email" htmlFor="email"><input id="email" name="email" type="email" required className="input" placeholder="you@example.com"/></Field><Field label="LinkedIn profile" htmlFor="linkedin"><input id="linkedin" name="linkedin" type="url" required className="input" placeholder="https://linkedin.com/in/…"/></Field><Field label="What can you actually speak to?" htmlFor="expertise"><textarea id="expertise" name="expertise" required rows={3} className="input" placeholder="Specific experience helps us match you well."/></Field><Field label="Your experience (optional)" htmlFor="experience"><input id="experience" name="experience" className="input" placeholder="E.g. 5 years, Senior PM at a startup"/></Field><Field label="Your career journey (optional)" htmlFor="journey"><textarea id="journey" name="journey" rows={3} className="input"/></Field><Field label="Why do you want to mentor? (optional)" htmlFor="why_mentor"><textarea id="why_mentor" name="why_mentor" rows={3} className="input"/></Field><Field label="Your booking link (Calendly or similar)" htmlFor="calendly"><input id="calendly" name="calendly" type="url" required className="input" placeholder="https://calendly.com/…"/></Field>{error&&<p className="text-sm text-red-700" role="alert">{error}</p>}<button disabled={submitting} className="w-full sm:w-auto inline-flex items-center justify-center rounded-sm bg-amber text-ink font-semibold px-7 py-3.5 disabled:opacity-60">{submitting?"Sending…":"Submit"}</button></form></div></main>;
}
