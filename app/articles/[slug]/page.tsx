...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const fallbackRelated = related.length < 2
    ? ARTICLES.filter((item) => item.slug !== article.slug && !related.some((r) => r.slug === item.slug)).slice(0, 2 - related.length)
    : [];
    ...
    <main className="min-h-screen bg-paper text-ink">
      ...
      <article>
        <div className="mx-auto max-w-5xl px-5 pb-14 pt-10 sm:px-6 sm:pb-20 sm:pt-14">
          <Link href="/articles" className="inline-flex items-center gap-2 font-mono text-xs uppercase tracking-[0.14em] text-board/70 hover:text-board"><ArrowLeft size={14} /> Back to articles</Link>
              ...
              <div className="mt-12 space-y-12">
                {article.sections.map((section) => (
                  <section key={section.heading}>
                    ...
                    <div className="mt-4 space-y-4 text-sm leading-7 text-ink/70 sm:text-base">{section.paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div>
                    {section.bullets && <ul className="mt-5 space-y-2 rounded-sm border border-board/10 bg-paper p-5 text-sm leading-6 text-ink/70">{section.bullets.map((bullet) => <li key={bullet} className="flex gap-3"><span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-amber" />{bullet}</li>)}</ul>}
                  </section>
                ))}
              </div>
              <div className="mt-12 rounded-sm border border-board/20 bg-board p-6 text-chalk sm:p-8"><p className="font-mono text-[10px] uppercase tracking-[0.18em] text-sage">The AglaKadam takeaway</p><p className="mt-4 font-display text-xl leading-relaxed sm:text-2xl">{article.takeaway}</p></div>
            </div>

            <aside className="lg:sticky lg:top-24 lg:self-start">
              <div className="rounded-sm border border-ink/10 bg-paper p-5"><p className="font-mono text-[10px] uppercase tracking-[0.16em] text-board/65">Make the next step practical</p><p className="mt-3 text-sm leading-6 text-ink/65">Have a question this article cannot answer? Talk to someone who has already navigated a similar decision.</p><Link href="/find-mentor" className="mt-5 inline-flex w-full items-center justify-center gap-2 rounded-sm bg-amber px-4 py-3 text-sm font-semibold">Find a mentor <ArrowRight size={15} /></Link></div>
              <div className="mt-4 flex gap-2"><Link href="/find-mentor" className="inline-flex flex-1 items-center justify-center gap-2 rounded-sm border border-ink/10 bg-white px-3 py-2.5 text-xs font-semibold"><MessageCircle size={14} /> Ask someone</Link><ShareArticleButton title={article.title} /></div>
            </aside>
          </div>
        </div>
      </article>
