...
export default function FeaturedMentors() {
  ...
  useEffect(() => {
    async function load() {
      const [{ data: mentorData }, { data: ratingData }] = await Promise.all([
        supabase
          .from("mentors")
          .select("id,name,headline,role,company,location,expertise,experience,bio,photo_url,calendly")
          .order("created_at", { ascending: false })
          .limit(3),
        supabase.from("mentor_ratings").select("mentor_id,rating"),
      ]);
      ...
      <div className="mx-auto max-w-6xl px-6 py-16 sm:py-20">
        <div className="mb-10 flex items-end justify-between gap-6">
          <div>
            ...
            <h2 className="font-display text-2xl sm:text-3xl max-w-xl">
        ...
        {loading ? (
          <div className="grid gap-6 sm:grid-cols-3">
            {[1, 2, 3].map((item) => (
              <div key={item} className="h-72 animate-pulse rounded-sm border border-ink/10 bg-paper" />
            ))}
          </div>
        ) : mentors.length === 0 ? (
          <div className="rounded-sm border border-dashed border-ink/15 bg-paper p-8 text-center">
            ...
            <Link href="/mentor" className="mt-5 inline-flex rounded-sm bg-amber px-5 py-2.5 text-sm font-semibold">
              Become a mentor
            </Link>
          </div>
        ) : (
          <div className="grid gap-6 sm:grid-cols-3">
            {mentors.map((mentor) => {
              ...
              const tags = mentor.expertise
                ? mentor.expertise.split(",").map((item) => item.trim()).filter(Boolean).slice(0, 3)
                : [];
              ...
              return (
                <article key={mentor.id} className="pin-shadow flex flex-col rounded-sm border border-ink/10 bg-paper p-6 transition-transform hover:-translate-y-1">
                  {mentor.photo_url ? (
                    <img src={mentor.photo_url} alt={`${mentor.name} profile`} className="h-20 w-20 rounded-full border border-ink/10 object-cover" />
                  ) : (
                    <div className="flex h-20 w-20 items-center justify-center rounded-full border border-board/10 bg-board/10 font-display text-3xl text-board">
                      {initial}
                    </div>
                  )}

                  <div className="mt-5">
                    <h3 className="font-display text-xl">{mentor.name}</h3>
                    <p className="mt-1 text-sm text-ink/60">{title}</p>
                  </div>

                  <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-xs text-ink/55">
                    {rating && (
                      <span className="inline-flex items-center gap-1"><Star size={13} className="fill-amber text-amber" /> {rating.average} ({rating.count})</span>
                    )}
                  ...
                  {tags.length > 0 && (
                    <div className="mt-4 flex flex-wrap gap-2">
                      {tags.map((tag) => <span key={tag} className="rounded-full bg-board/10 px-3 py-1 text-xs text-board">{tag}</span>)}
                    </div>
                  )}

                  <p className="mt-4 flex-1 text-sm leading-relaxed text-ink/65">
                    {mentor.bio || "Experience and practical guidance for your next step."}
                  </p>

                  <div className="mt-6 flex flex-wrap gap-2">
                    <Link href={`/mentors/${mentor.id}`} className="inline-flex items-center justify-center rounded-sm bg-amber px-4 py-2.5 text-sm font-semibold">
                      View profile
                    </Link>
                    {mentor.calendly && (
                      <a href={mentor.calendly} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 rounded-sm border border-ink/20 px-4 py-2.5 text-sm hover:bg-ink/5">
        ...
        <Link href="/mentors" className="mt-8 inline-flex items-center gap-1 text-sm font-semibold text-board sm:hidden">
