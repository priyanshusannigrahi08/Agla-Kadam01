...
function AuthForm() {
  ...
  async function sendCode(e: FormEvent) {
    ...
    const result = mode === "email"
      ? await supabase.auth.signInWithOtp({ email: value, options: { shouldCreateUser: true, emailRedirectTo: callbackUrl() } })
      ...
      <div className="w-full max-w-md bg-white border border-ink/10 shadow-sm p-7 sm:p-10">
        ...
        {!sent ? <>
          ...
          <div className="flex gap-2 mt-6 mb-4 text-sm">
            <button type="button" onClick={() => { setMode("email"); setMessage(""); }} className={`flex-1 py-2 border ${mode === "email" ? "border-amber bg-paper" : "border-ink/15"}`}>Email OTP</button>
            <button type="button" onClick={() => { setMode("phone"); setMessage(""); }} className={`flex-1 py-2 border ${mode === "phone" ? "border-amber bg-paper" : "border-ink/15"}`}>Phone OTP</button>
          </div>
          <form onSubmit={sendCode} className="space-y-4">
            ...
            <button disabled={loading} className="w-full bg-amber py-3 font-semibold disabled:opacity-60">{loading ? "Sending…" : "Send verification code"}</button>
          </form>
          ...
          <button disabled={loading} className="w-full bg-amber py-3 font-semibold disabled:opacity-60">{loading ? "Verifying…" : "Verify and sign in"}</button>
          <button type="button" onClick={() => { setSent(false); setOtp(""); }} className="w-full text-sm underline">Use a different method</button>
        </form>}
...
export default function AuthPage() {
  return (
    <Suspense fallback={<main className="min-h-screen bg-paper text-ink flex items-center justify-center px-6 py-16"><div className="w-full max-w-md bg-white border border-ink/10 shadow-sm p-7 sm:p-10"><p className="text-sm text-ink/60">Loading sign in…</p></div></main>}>
      <AuthForm />
    </Suspense>
  );
}
