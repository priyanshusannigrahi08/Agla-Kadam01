...
create table if not exists reviews (
  ...
  mentor_id uuid not null references mentors(id) on delete cascade, reviewer_user_id uuid not null,
  reviewer_name text not null, rating integer not null check (rating between 1 and 5), comment text,
  status text not null default 'published', unique (mentor_id, reviewer_user_id)
);
create table if not exists bookings (
  id uuid primary key default gen_random_uuid(), created_at timestamptz not null default now(),
  mentee_user_id uuid not null, mentor_id uuid not null references mentors(id) on delete cascade,
  scheduled_for timestamptz, duration_minutes integer not null default 30 check (duration_minutes = 30),
  status text not null default 'requested', booking_url text, notes text
);
...
create policy "Public can read published reviews" on reviews for select to anon, authenticated using (status = 'published');
drop policy if exists "Users can create own reviews" on reviews;
create policy "Users can create own reviews" on reviews for insert to authenticated with check (reviewer_user_id = auth.uid());
drop policy if exists "Users can create own bookings" on bookings;
create policy "Users can create own bookings" on bookings for insert to authenticated with check (mentee_user_id = auth.uid());
drop policy if exists "Users can read own bookings" on bookings;
create policy "Users can read own bookings" on bookings for select to authenticated using (mentee_user_id = auth.uid());
drop policy if exists "Mentors can read their bookings" on bookings;
create policy "Mentors can read their bookings" on bookings for select to authenticated using (exists (select 1 from mentors m where m.id = bookings.mentor_id and m.user_id = auth.uid()));
drop policy if exists "Mentors can update their bookings" on bookings;
create policy "Mentors can update their bookings" on bookings for update to authenticated using (exists (select 1 from mentors m where m.id = bookings.mentor_id and m.user_id = auth.uid()));
drop policy if exists "Users can read own mentor profile" on mentors;
create policy "Users can read own mentor profile" on mentors for select to authenticated using (user_id = auth.uid());
...
select id, created_at, name, headline, bio, expertise, experience, company, role,
  location, linkedin, linkedin_url, calendly, calendly_url, photo_url, availability, verification_status
from mentors where status = 'approved';
