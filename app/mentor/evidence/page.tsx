...
function DiscoveryField({ title, values }: { title: string; values?: string[] }) { const items = (values || []).filter(Boolean); return <div className="rounded-sm border border-ink/10 bg-white p-4"><p className="text-sm font-semibold">{title}</p>{items.length ? <div className="mt-3 flex flex-wrap gap-2">{items.map((item) => <span key={item} className="rounded-full border border-board/15 bg-board/5 px-3 py-1.5 text-xs text-board">{item}</span>)}</div> : <p className="mt-2 text-xs text-ink/40">Nothing clearly extracted.</p>}</div>; }
